...
#    final states, BFS performs a blind search
#    on the graph to find one of the final states.
# 2. It chooses to expand the set-front with all states
#    that are at the same depth and then proceeds to
#    higher level states.
...
# Creating the BFS_Graph_Search() function with graph, start and goals arguments. This function represents
# the Breadth First Search (BFS) algorithm applied to the graph (1st argument), in order to search for a smart 
...
def BFS_Graph_Search(graph, start, goals): # Creating the function BFS_Graph_Search(graph, start, goals).

    queue = [(start, [start])]  # Initializing the queue as a list containing a tuple with the start node
    ...
    visited = []   # Initializing an empty list named visited to
                   # keep track of the nodes that BFS has visited.

    # Starting a while loop that will continue as long as the queue is NOT empty.
    while queue:
        node, path = queue.pop(0)   # Removes and returns the leftmost element ".pop(0)" from the queue,
        ...
        for neighbor in graph[node]:    
            if neighbor not in visited:  # Checking if the neighbor has not already been visited and is therefore not in the visited list.
                ...
                queue.append((neighbor, new_path))  # Finally, the new tuple is inserted into the queue, representing the node
...
# Calls BFS_Graph_Search() with arguments: myGraph (the graph), initial_state (the initial state I)
...
# 3. All nodes in order visited by the algorithm, regardless of whether they belong to the solution path.
target_node, graph_path, visited_order = BFS_Graph_Search(myGraph, initial_state, goal_states)
...
if visited_order is not None:
    print("Target node found:", target_node) # Printing the target node found by BFS.
    ...
    print("All Nodes visited in order:", visited_order) # Printing all nodes in order that were visited by BFS, regardless of whether they belong to the solution path.
